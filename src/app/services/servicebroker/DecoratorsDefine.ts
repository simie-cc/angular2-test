...
//@ResuestMapping({path:"/hello", method: HttpMethod.GET})
export function RequestMapping(options:{path:string,method:HttpMethod}){
  //console.log('RestMapping:'+JSON.stringify(options)) ;
  //target 會是掛載的method所屬的Class物件的prototype
  return Reflect.metadata(REST_METADATA, { path:options.path, method:options.method }) ;
}
